...
function getCurrentImageOfTheDay() {
  ...
  // console.log(date);
  fetch(
    `https://api.nasa.gov/planetary/apod?api_key=lif0dYtK9xpLwnIBOu9WL80rFT0Pn5oeVremXK2W&date=${date}`
  )
    .then((res) => {
      return res.json();
    })
...
function getImageOfTheDay() {
  ...
  // alert(newDate);
  fetch(
    `https://api.nasa.gov/planetary/apod?api_key=lif0dYtK9xpLwnIBOu9WL80rFT0Pn5oeVremXK2W&date=${newDate}`
  )
    .then((res) => {
      return res.json();
    })
...
function getUserDate(year, month, date) {
  ...
  fetch(
    `https://api.nasa.gov/planetary/apod?api_key=dmylZQirmdOEA2EO1mdEuEl3tnTjfffH9I8Ccj4Q&date=${year}-${month}-${date}`
  )
    .then((res) => {
      return res.json();
    })
